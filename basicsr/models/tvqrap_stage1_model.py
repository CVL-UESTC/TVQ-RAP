...
class TVQRAP_stage1_Model(SRModel):
    ...
    def init_training_settings(self):
        ...
        self.disc_weight = train_opt.get('disc_weight', 0.75)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, l_codebook, quant_stats,x_hr,x_lr,img_lr = self.net_g(self.gt,self.lq)
        ...
        if current_iter >= self.net_d_start_iter:
            ...
            last_layer = self.net_g.module.generator.blocks_3[-1].weight
        ...
        # pixel loss
        if self.cri_pix:
            l_g_pix = self.cri_pix(img_lr, self.lq)
            l_g_total += l_g_pix
            loss_dict['l_g_pix_lr'] = l_g_pix

        l_align = torch.mean(    ((x_hr-x_lr.detach())**2) )  
        l_g_total += l_align
        loss_dict['l_align'] = l_align

        loss_dict['distance'] =  torch.mean(   torch.sqrt((x_hr-x_lr)**2)   ) 
        loss_dict['value'] = torch.mean(torch.sqrt(x_hr**2)   ) 
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            out_img = tensor2img([visuals['result']], min_max=[-1,1])
               

            if save_img:
                save_img_out_path = osp.join(self.opt['path']['visualization'],str(current_iter),"fake",
                                        f'{img_name}_out.png')
                imwrite(out_img, save_img_out_path)     
