...
def create_train_val_dataloader(opt, logger,finetune_gt_size = False):
    # create train and val dataloaders
    train_loader, val_loader = None, None
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            total_iters = int(opt['train']['total_iter'])
            dataset_opt = copy.deepcopy(opt['datasets']['train'])
            if dataset_opt.get('finetune_gt_size') and ( finetune_gt_size is True ):
                dataset_opt['gt_size'] = dataset_opt['finetune_gt_size']
                dataset_opt['batch_size_per_gpu'] = dataset_opt['finetune_batch_size_per_gpu']
                total_iters = int(opt['train']['finetune_total_iter'])
...
def train_pipeline(root_path):
    ...
    if opt['datasets']['train'].get('finetune_gt_size'):
        # 2. 清理旧的 loader 释放内存
        del train_loader, train_sampler, prefetcher
        torch.cuda.empty_cache()
        model.reset_queue()

        # create train and validation dataloaders
        result = create_train_val_dataloader(opt, logger, finetune_gt_size = True)
        train_loader, train_sampler, val_loader, total_epochs, total_iters = result
        start_epoch = 0
        current_iter = 0
        # create message logger (formatted outputs)
        ...
        # training
        logger.info(f'Start finetune training (512x512 image_size)')
        data_time, iter_time = time.time(), time.time()
        start_time = time.time()
